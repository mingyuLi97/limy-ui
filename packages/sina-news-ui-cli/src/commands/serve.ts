...
import { getWebsiteDevConfig } from '../config/webpack.website.dev';
import { getExampleDevConfig } from '../config/webpack.example.dev';
...
export default async function serve(target: ServeTarget) {
  console.log(`[serve] target111`, target);
  const webpackConfig =
